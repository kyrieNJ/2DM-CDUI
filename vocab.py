...
if __name__ == '__main__':
    # fin = open('alltext.raw', 'r', encoding='utf-8', newline='\n', errors='ignore')
    # alltext = fin.readlines()
    # alltext=alltext[0]
    # fin.close()
    #
    # # tmpkey=alltext.split()
    # tmpkey=jieba.lcut(alltext,cut_all=False)
    # counter={}
    # for i in range(len(tmpkey)):
    #     if tmpkey[i] not in counter.keys():
    #         counter[tmpkey[i]]=1
    #     else:
    #         counter[tmpkey[i]]+=1
    # counter['<pad>']=1
    ...
    # njsonfile = '../datasets/normal_NoAscii.json'
    # tmpdict = parse_json_all(njsonfile)
    # for idct in tqdm(tmpdict):
    #     tmp_text=''
    #     tmp_text+=' '+idct["label"]
    #     tmp_text+=' '+idct["nickname"]
