...
from data_utils import MM_PDUDDataset
...
class Instructor:
    def __init__(self, args):
        ...
        #数据加载阶段
        self.trainset = MM_PDUDDataset(args.dataset_file['train'],args)
        self.testset = MM_PDUDDataset(args.dataset_file['test'],args)
        #模型训练阶段
    ...
    def _train(self, criterion, optimizer,bestrecord_list):
        max_test_acc = 0
        max_test_f1 = 0
        global_step = 0
        ...
        for epoch in range(self.args.num_epoch):
            ...
            tmpstr='loss: {:.4f}, acc: {:.4f}, \n test_acc: {:.4f}, test_f1: {:.4f}'.format(loss.item(), train_acc, test_acc, test_f1)
            bestrecord_list.append(tmpstr)
        ...
        return max_test_acc, max_test_f1,bestrecord_list
    ...
    def run(self):
        ...
        f_out = open('log/' + self.args.model_name + '_' + self.args.dataset + '_val_MM_RP.txt', 'w', encoding='utf-8')
        bestrecord_list=[]
        f_out2 = open('log/' + self.args.model_name + '_' + self.args.dataset + '_best_record.txt', 'w', encoding='utf-8')
        ...
        for i in range(args.repeat):
            ...
            self._reset_params()
            max_test_acc, max_test_f1,bestrecord_list = self._train(criterion, self.optimizer,bestrecord_list)
        ...
        for i in range(len(bestrecord_list)):
            f_out2.writelines('------------------------------------------------------------------------\n')
            f_out2.writelines(bestrecord_list[i])
        f_out2.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='2DM-CDUI', type=str)
    parser.add_argument('--dataset', default='SWDD_1k8', type=str, help='')
    parser.add_argument("--hidden_dim", type=int, default=768, help="bert dim.")
    parser.add_argument("--num_class", type=int, default=2, help="Num of fsentiment class.")
    ...
    parser.add_argument("--seed", type=int, default=2) #2 15
    ...
    parser.add_argument("--twi_max_len", type=int, default=100)
    ...
    parser.add_argument('--sim_v', default=0.85, type=str)
